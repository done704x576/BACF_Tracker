// BACF_Tracker.cpp : 定义控制台应用程序的入口点。
//
...
struct TrackedRegion 
{
	...
	TrackedRegion(const cv::Rect box) : center(box.x + (int)((float)box.size().width / 2.0),
		box.y + (int)((float)box.size().height / 2.0)),
		size(box.size()) { }
	...
	TrackedRegion resize(const float factor) const 
	{
		...
		newRegion.size = cv::Size((int)(size.width * factor), (int)(size.height*factor));
		return newRegion;
	}
...
bool BACF::initImpl(const Mat& image, const Rect& boundingBox) 
{
	initialize(image, boundingBox);
	return true;
}

bool BACF::updateImpl(const Mat& image, Rect& boundingBox) 
{
	detect(image);
...
{
	//filter settings
	p.model_sz = cv::Size(50, 50);
	p.target_padding = 2.0;
	//learning parameters
	p.update_rate = 0.003;
	p.sigma_factor = 1.0 / 16.0;
...
void BACF::detect(const cv::Mat& image) 
{
	...
	cv::Point2i translation((int)(shift_index(maxpos.x, response.cols)*scale_factor),
		(int)(shift_index(maxpos.y, response.rows)*scale_factor));
...
cv::Mat BACF::extractTrackedRegionSpec(cv::Mat model, const cv::Size output_sz)
{
	...
	cv::Mat	lp = extractTrackedRegion(model, tr, Size(output_sz.width / 2, output_sz.height / 2));
	lp.copyTo(lp1(r));
	//std::cout << lp1;
	return lp1;
}
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//std::string video = "E:\\CarRearVideo\\7.mp4";
	...
	Rect roi = box.extract("tracker", frame);
